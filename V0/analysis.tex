...
The low E 2D signal model is computed according to Eq.~\ref{eq:low2D}
\begin{multline}
\label{eq:low2D}
  \frac{\mathrm{d}^2 R}{\mathrm{d}cS1.\mathrm{d}cS2} = \epsilon_\mathrm{S1}(cS1).\epsilon_\mathrm{S2}(cS2) \\ 
  \times \int \! \frac{\mathrm{d}R}{\mathrm{d}E}.p_\mathrm{cS1}(\mathrm{cS1}|E).p_\mathrm{cS2}(\mathrm{cS2}|E) \, \mathrm{d}E
\end{multline}

where

\begin{align}
 p_\mathrm{cS1}(\mathrm{cS1}|E) &= \sum_{N'} P_\mathrm{pmt}(\mathrm{cS1}|N',0.5 \sqrt{N'}).\mathrm{Pois}(N'|\mu_\gamma) \\
 p_\mathrm{cS2}(\mathrm{cS2}|E) &= \sum_{N'} P_\mathrm{pmt}(\mathrm{cS2}|Y N',\sigma_Y \sqrt{N'}).\mathrm{Pois}(N'|\mu_Q)
\end{align}
...
with fixed parameters $L_y = 2.28$, $S_\mathrm{nr} = 0.95$, $S_\mathrm{ee} = 0.58$, $Y = 8.4366$, and $\sigma_Y = 6.93$.
