...
\label{subsec:LowE}
This analysis channel \sout{takes advantage of} \RanComment{relies on} the re-analysis of run~II data described in~\cite{xe100_run_combination}. The region of interest, the background 
expectation models, data selections and their acceptances are mostly unchanged \RanComment{and only briefly summarized here. Differances with respect to said results are highlighted when present.} \sout{with respect to said result and only briefly summarized here, 
...
relative acceptance \sout{on} \RanComment{of} WIMP signals can be found in~\cite{Aprile:2012vw,xe100_run_combination}. 
