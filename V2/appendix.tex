...
\label{app:1000PE}
This analysis is focused on energy recoils up to 240\,keV (180\,PE) since due to low statistics of NR calibration data higher energies are not considered. For completeness, the data for events with recoil energy up to 1000\,PE is presented in ~\ref{fig:eft_1000}. The data selection criteria are the same as those applied for the high-energy region. \RanComment{DM data is compatible with background calibration one, and no abnormalities have been seen.}
...
In this appendix we describe digital tables which can be used to construct an accurate signal model for this analysis given any input recoil spectrum $\mathrm{d}R/\mathrm{d}E$ arising from a theoretical model. A visualization of the tables is shown in Fig. \ref{fig:smeartable_highE}, and in section \ref{app:example_code} we show a simple example Python code of how to use the supplied tables. Currently we provide these tables only for the high E analysis region.
