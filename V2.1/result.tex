...
Furthermore we examinated the region in \cSi{} above 180\,PE and up to 1000\,PE. We used the same data selection criteria as those applied for the high-energy channel. 
...
For the elastic operators $O_1$ and $O_4$, our results can be compared to those of standard SI/SD analyses by computing the relevant zero-momentum WIMP-nucleon cross-sections. This is not simple to do rigorously because the treatment of nuclear structure used in our analysis is different than in standard analyses, however this difference is small for scattering via $O_1$ and $O_4$. We can therefore quite safely use the `traditional' correspondences. For $O_1$ this is
%
\begin{equation}
\sigma_{N}^\mathrm{SI} = \left(C^N_1\right)^2 \frac{\mu_{\chi,N}^2}{\pi}
\end{equation}
%
where $\mu_{\chi,N}$ is the WIMP-nucleon reduced mass. Standard SI analyses assume isospin-conserving interactions, as we do in this analysis, so we can simply set $C^N_1 = C^0_1$, such that $\sigma_{p}^\mathrm{SI}=\sigma_{n}^\mathrm{SI}$. Similarly, for $SD$ interactions the usual correspondence between EFT coupling and zero-momentum cross-section is
%
\begin{equation}
\sigma_{N}^\mathrm{SD} = \left(C^N_1\right)^2 \frac{3 \mu_{\chi,N}^2}{\pi}
\end{equation}
%
however standard SD analysis do {\em not} assume isospin-conserving interactions; they typically derive separate limits for $\sigma_{p}^\mathrm{SD}$ and $\sigma_{n}^\mathrm{SD}$, assuming that WIMPs couple only to either protons or neutrons respectively.
