...
and its asymptotic distributions~\cite{asympt}. \RanComment{The two analysis channels are combined by multiplying their likelihoods together to produce a joint likelihood, this is possible because they are statistically independent once certain nuisance parameters are factored out}. Both analyses parametrize the NR relative scintillation efficiency, $\Leff$, based on existing measurements~\cite{run8Result}. Its uncertainty is the major contributor to energy scale uncertainties and is considered as correlated between the two analysis channels via a separate nuisance likelihood term.
