...
The statistical interpretation of data is performed \BenComment{using} a binned profile likelihood method, in which hypothesis testing relies upon a likelihood ratio test statistic, $\tilde{q}$, 
and its asymptotic distributions~\cite{asympt}. \RanComment{The two analysis channels are combined by multiplying their likelihoods together to produce a joint likelihood. This is possible because the two channels are statistically independent once certain nuisance parameters are factored out}. Both analyses parametrize the NR relative scintillation efficiency, $\Leff$, based on existing measurements~\cite{run8Result}. Its uncertainty is the major contributor to energy scale uncertainties and is considered as correlated between the two analysis channels via a separate nuisance likelihood term.
